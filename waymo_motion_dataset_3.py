...
def collate(samples):
    ...
    roadgraph_polylines_padded.new_tensor(-1e6)
...
#         print('traffic_lights_polylines[traffic_lights_polylines_mask]', traffic_lights_polylines[traffic_lights_polylines_mask, :].shape)
        ...
        if np.sum(objects_of_interest == 1) < 2:
            if np.sum(tracks_to_predict == 1) < 2:
                ...
                indices = rng.choice(np.argwhere(np.sum(parsed['state/future/valid'],axis = 1)>=78), size = 2, replace = False, p = None)
                objects_of_interest[indices] = 1

                
        # change origin
       
        index = tf.where(objects_of_interest==1)
        ...
        origin_Y = origin[1]
        origin_Z = origin[2]
        past_X = parsed['state/past/x'].numpy()- origin_X
        past_Y = parsed['state/past/y'].numpy()- origin_Y
        past_Z = parsed['state/past/z'].numpy()- origin_Z
        current_X = parsed['state/current/x'].numpy()- origin_X
        current_Y = parsed['state/current/y'].numpy() - origin_Y
        current_Z = parsed['state/current/z'].numpy()- origin_Z
        future_X = parsed['state/future/x'].numpy() - origin_X
        future_Y = parsed['state/future/y'].numpy() - origin_Y
        future_Z = parsed['state/future/z'].numpy() - origin_Z
        ...
        roadgraph_vecs = np.concatenate([parsed['roadgraph_samples/xyz'] - origin,
